...
public class VoiceTrafficCallManager extends InteraxTelephonyCallManager {
	
	private VoiceTrafficAmi voiceTrafficAmi;
	private VoiceTrafficCallManagerLogger managerLogger;
	private int currentSleepTime;
	private Calendar nextWakeUpDate;
	private String backupFileName;
	protected VoiceTrafficManagedCall managedCall;
	private String childrenCdrId;
		
	public VoiceTrafficCallManager(VoiceTrafficAmi voiceTrafficAmi, VoiceTrafficManagedCall managedCall, AsteriskChannel incomingChannel, AsteriskChannel outgoingChannel){
		super(incomingChannel, outgoingChannel, voiceTrafficAmi.getVoiceTrafficAmiConfig().SLEEP_TIME, voiceTrafficAmi.getVoiceTrafficAmiConfig().SAFETY_SECONDS_TO_HANGUP, voiceTrafficAmi.getGenericEjbCaller());
		this.managedCall = managedCall;
		this.voiceTrafficAmi = voiceTrafficAmi;
		this.managerLogger = new VoiceTrafficCallManagerLogger(this,this.voiceTrafficAmi.getConfigPath());
	}
    ...
    public void run(){
    	...
    	this.managerLogger.info("/******/Call data/******/");
      	...
      	this.backupFileName = VoiceTrafficAmi.BACKUP_FILE_PREFIX + this.managedCall.getIncomingCdrId(); 
    ...
    private void getNewBlock(boolean calculateNextWakeUpDate){
			...
			if(calculateNextWakeUpDate){
				...
				if(secondsLeft < ServiceReservation.BASE_TIME_IN_SECONDS){
					...
					try {
						...
						this.voiceTrafficAmi.managerConnection.sendAction(streamAction,0);
					} catch (Exception e1) {
    	...
    	} catch (VoiceTrafficNotEnoughBalanceException e) {
			// No more money for this call
			this.toHangup = true;
			this.managerLogger.warn("There is no more money available for this call.");
			this.managerLogger.warn("Detail: " + e.getMessage());
	...
	private Integer getBlock(long reservationId) throws InteraxTelephonyException {
		...
		return getBlock(reservationId, this.voiceTrafficAmi.getVoiceTrafficAmiConfig().RATER_EJB_SERVER, this.voiceTrafficAmi.getVoiceTrafficAmiConfig().RATER_EJB_PORT);
	}
	...
	private boolean getBlock(long reservationId, long blocks) throws InteraxTelephonyException {
		...
		return getBlock(reservationId, blocks, this.voiceTrafficAmi.getVoiceTrafficAmiConfig().RATER_EJB_SERVER, this.voiceTrafficAmi.getVoiceTrafficAmiConfig().RATER_EJB_PORT);
	}
