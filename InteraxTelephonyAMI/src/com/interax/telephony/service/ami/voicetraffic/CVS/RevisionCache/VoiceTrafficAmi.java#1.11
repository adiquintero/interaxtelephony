...
public class VoiceTrafficAmi extends InteraxTelephonyAmiServer{
    
	public static final String BACKUP_FILE_PREFIX = "voice-traffic-";
	private Thread amiThread;
	private static ServiceFamily serviceFamily = ServiceFamily.VOICE_TRAFFIC;
	private VoiceTrafficAmiLogger logger;
	private InteraxTelephonyAmiServerConfig interaxTelephonyAmiConfig;
	private VoiceTrafficAmiConfig voiceTrafficAmiConfig;
    private VoiceTrafficAccessType accessType;
	private String configPath = null;
    private String persistencePath ;
	private String onlineCallPersistenceFolder;

    public VoiceTrafficAmi(){
    	super();
    	
    	this.amiThread = Thread.currentThread();
		this.amiThread.setName("VoiceTrafficAmi");
		interaxTelephonyAmiConfig = InteraxTelephonyAmiServerConfigLoader.getConfig();
		this.configPath = interaxTelephonyAmiConfig.CONFIG_PATH + interaxTelephonyAmiConfig.AMIS_CONFIG_PATH + "/voicetraffic/";
	    ...
	    this.logger = new VoiceTrafficAmiLogger(this, this.configPath);
		...
		this.logger.info("Recovering VoiceTraffic persistent calls.");
		for (Object object : managedCalls) {
			VoiceTrafficManagedCall managedCall = (VoiceTrafficManagedCall)object;
			AsteriskChannel incomingChannel = null;
			AsteriskChannel outgoingChannel = null;
			if(managedCall!=null){
				this.logger.debug("Persistent Call:", managedCall);
				voiceTrafficAmiConfig = (VoiceTrafficAmiConfig)VoiceTrafficAmiConfigLoader.getConfig(managedCall.getEnterpriseId(), this.configPath);
				try {
					...
					if(incomingChannel != null && outgoingChannel != null && outgoingChannel.getState() == ChannelState.UP 
						...
						VoiceTrafficCallManager callManager = new VoiceTrafficCallManager(this, managedCall, incomingChannel, outgoingChannel);
						callManager.start();
    ...
    public void onManagerEvent(ManagerEvent event){
    	Thread.currentThread().setName("VoiceTrafficAmi");
    	...
    	if(event instanceof LinkEvent){
    		...
    		try{
	    		...
	    		if (enterprise != null && !enterprise.equals("(null)")){
	    			...
	    			voiceTrafficAmiConfig = (VoiceTrafficAmiConfig)VoiceTrafficAmiConfigLoader.getConfig(enterpriseId,this.configPath);
	    			
		    		if(this.voiceTrafficAmiConfig == null){
	    				this.logger.info("No config file associated to this enterprise.");
	    				return;
	    			}
		    		this.logger.info("VoiceTraffic Ami Config loaded");
	    			ServiceFamily currentServiceFamily = ServiceFamily.valueOf(incomingChannel.getVariable(serviceFamily.name() + "_SERVICE_FAMILY"));
		    		if(currentServiceFamily!=null && currentServiceFamily == ServiceFamily.VOICE_TRAFFIC){
		    			this.logger.info("VoiceTraffic Call detected.");
		    			accessType = VoiceTrafficAccessType.valueOf(incomingChannel.getVariable(serviceFamily.name() + "_ACCESS_TYPE"));
		    			...
		    			this.onlineCallPersistenceFolder = persistencePath + "agis/voicetraffic/VoiceTraffic" + StringUtils.toCamelCase(accessType.name().toLowerCase()) + "Agi/onlineCall/";
		    			...
		    			switch (accessType) {
							...
							case VON:
								...
								this.logger.info("VoiceTraffic outgoing Call detected for incoming channel " + incomingUniqueId);
								...
								if((unlimitedCall!=null && unlimitedCall.equals("TRUE")) || (unmonitoredCall!=null && unmonitoredCall.equals("TRUE"))){
					    			...
					    			VoiceTrafficManagedCall managedCall = new VoiceTrafficManagedCall();
...
//					    			RATER_TEST_MODE:1
    ...
    public static void main(String[] args) throws Exception{
    	...
    	voiceTrafficAmi = new VoiceTrafficAmi();
    	voiceTrafficAmi.run();
    }

	public VoiceTrafficAmiLogger getLogger() {
		return logger;
	}
	...
	public VoiceTrafficAmiConfig getVoiceTrafficAmiConfig() {
		return voiceTrafficAmiConfig;
	}
