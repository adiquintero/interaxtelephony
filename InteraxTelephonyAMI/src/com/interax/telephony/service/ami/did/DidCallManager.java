...
import com.interax.telephony.service.ami.did.DidCallManagerLogger;
...
import com.interax.telephony.service.exception.did.DidNotEnoughBalanceException;
import com.interax.telephony.util.PersistenceManager;
import com.interax.telephony.util.StackTrace;

public class DidCallManager extends InteraxTelephonyCallManager {
	
	private DidAmi didAmi;
	private DidCallManagerLogger managerLogger;
	private int currentSleepTime;
	private Calendar nextWakeUpDate;
	private String backupFileName;
	protected DidManagedCall managedCall;
	private String childrenCdrId;
		
	public DidCallManager(DidAmi didAmi, DidManagedCall managedCall, AsteriskChannel incomingChannel, AsteriskChannel outgoingChannel){
		super(incomingChannel, outgoingChannel, didAmi.getDidAmiConfig().SLEEP_TIME, didAmi.getDidAmiConfig().SAFETY_SECONDS_TO_HANGUP, didAmi.getGenericEjbCaller());
		this.managedCall = managedCall;
		this.didAmi = didAmi;
		this.managerLogger = new DidCallManagerLogger(this,this.didAmi.getConfigPath());
	}
    ...
    public void run(){
    	...
    	this.managerLogger.info("/******/Call data/******/");
      	...
      	this.backupFileName = DidAmi.BACKUP_FILE_PREFIX + this.managedCall.getIncomingCdrId(); 
    ...
    private void getNewBlock(boolean calculateNextWakeUpDate){
			...
			if(calculateNextWakeUpDate){
				...
				if(secondsLeft < ServiceReservation.BASE_TIME_IN_SECONDS){
					...
					try {
						...
						this.didAmi.managerConnection.sendAction(streamAction,0);
					} catch (Exception e1) {
    	...
    	} catch (DidNotEnoughBalanceException e) {
			// No more money for this call
			this.toHangup = true;
			this.managerLogger.warn("There is no more money available for this call.");
			this.managerLogger.warn("Detail: " + e.getMessage());
	...
	private Integer getBlock(long reservationId) throws InteraxTelephonyException {
		...
		return getBlock(reservationId, this.didAmi.getDidAmiConfig().RATER_EJB_SERVER, this.didAmi.getDidAmiConfig().RATER_EJB_PORT);
	}
	...
	private boolean getBlock(long reservationId, long blocks) throws InteraxTelephonyException {
		...
		return getBlock(reservationId, blocks, this.didAmi.getDidAmiConfig().RATER_EJB_SERVER, this.didAmi.getDidAmiConfig().RATER_EJB_PORT);
	}
