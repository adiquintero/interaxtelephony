...
public class IpPbxCallManager extends InteraxTelephonyCallManager {
	
	private IpPbxAmi ipPbxAmi;
	private IpPbxCallManagerLogger managerLogger;
	private int currentSleepTime;
	private Calendar nextWakeUpDate;
	private String backupFileName;
	private String childrenCdrId;
	protected IpPbxManagedCall managedCall;
		
	public IpPbxCallManager(IpPbxAmi ipPbxAmi, IpPbxManagedCall managedCall, AsteriskChannel incomingChannel, AsteriskChannel outgoingChannel){
		super(incomingChannel, outgoingChannel, ipPbxAmi.getIpPbxAmiConfig().SLEEP_TIME, ipPbxAmi.getIpPbxAmiConfig().SAFETY_SECONDS_TO_HANGUP, ipPbxAmi.getGenericEjbCaller());
		this.managedCall = managedCall;
		this.ipPbxAmi = ipPbxAmi;
		this.managerLogger = new IpPbxCallManagerLogger(this,this.ipPbxAmi.getConfigPath()); 
	}
    ...
    public void run(){
    	...
    	this.managerLogger.info("/******/Call data/******/");
      	...
      	this.backupFileName = IpPbxAmi.BACKUP_FILE_PREFIX + this.managedCall.getIncomingCdrId(); 
    ...
    private void getNewBlock(boolean calculateNextWakeUpDate){
    		...
    		if(calculateNextWakeUpDate){
				...
				if(secondsLeft < ServiceReservation.BASE_TIME_IN_SECONDS){
					...
					try {
						...
						this.ipPbxAmi.managerConnection.sendAction(streamAction,0);
					} catch (Exception e1) {
    	...
    	} catch (IpPbxNotEnoughBalanceException e) {
			// No more money for this call
			this.toHangup = true;
			this.managerLogger.warn("There is no more money available for this call.");
			this.managerLogger.warn("Detail: " + e.getMessage());
	...
	private Integer getBlock(long reservationId) throws InteraxTelephonyException {
		...
		return getBlock(reservationId, this.ipPbxAmi.getIpPbxAmiConfig().RATER_EJB_SERVER, this.ipPbxAmi.getIpPbxAmiConfig().RATER_EJB_PORT);
	}
	...
	private boolean getBlock(long reservationId, long blocks) throws InteraxTelephonyException {
		...
		return getBlock(reservationId, blocks, this.ipPbxAmi.getIpPbxAmiConfig().RATER_EJB_SERVER, this.ipPbxAmi.getIpPbxAmiConfig().RATER_EJB_PORT);
	}
