...
public class VoiceTrafficForwardAgi extends VoiceTrafficBaseAgi{
	...
	private VoiceTrafficForwardAgiLogger logger;
	private VoiceTrafficForwardAgiStep step;
	private static ServiceFamily serviceFamily = ServiceFamily.VOICE_TRAFFIC;
	private static VoiceTrafficAccessType accessType = VoiceTrafficAccessType.FORWARD;
	...
	private VoiceTrafficForwardAgiConfig config = null;
	...
	public void service(AgiRequest arg0, AgiChannel arg1) throws AgiException {
		...
		this.step = VoiceTrafficForwardAgiStep.INIT;
		this.agiThread = Thread.currentThread();
		this.agiThread.setName("VoiceTrafficForwardAgiThread-" + agiThread.getId());
		...
		this.logger = new VoiceTrafficForwardAgiLogger(this, InteraxTelephonyAgiServer.getConfig().CONFIG_PATH + InteraxTelephonyAgiServer.getConfig().AGIS_CONFIG_PATH + "/voicetraffic/forward/");
		...
		String persistenceFolder = InteraxTelephonyAgiServer.getConfig().PERSISTENCE_PATH + InteraxTelephonyAgiServer.getConfig().AGIS_CONFIG_PATH + "/" + serviceFamily.name().toLowerCase().replace("_", "") + "/" + VoiceTrafficForwardAgi.class.getSimpleName(); 
		...
		this.logger.info("VoiceTrafficForwardAgi started");
		this.step = VoiceTrafficForwardAgiStep.INIT_RESOURCES;

		//		 Máquina de estados
		while(this.step != VoiceTrafficForwardAgiStep.STOP){
		...
		this.logger.info("VoiceTrafficForwardAgi stopped");
	}
	...
	private void initResources(){
		...
		this.step = VoiceTrafficForwardAgiStep.PROCESS_CALL_DATA;
		//		}
	...
	private void processCallData(){
		try{
			...
			this.config = VoiceTrafficForwardAgiConfigLoader.getConfig(this.enterpriseId ,InteraxTelephonyAgiServer.getConfig().CONFIG_PATH + InteraxTelephonyAgiServer.getConfig().AGIS_CONFIG_PATH + "/voicetraffic/forward/");

			if(this.config==null){
				this.logger.info("No config file found.");
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
...
//			this.ani = this.getVariable("CALLERID(num)");
			...
			this.step = VoiceTrafficForwardAgiStep.VALIDATE_DESTINY;
	...
	private void validateDestiny(){
		...
		try{
			...
			try
			{
				...
				this.step = VoiceTrafficForwardAgiStep.MAKE_CALL;
			}
				...
				streamFile(VoiceTrafficForwardAgiAudio.CALL_INVALID_DESTINY);
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
				...
				streamFile(VoiceTrafficForwardAgiAudio.CALL_RESTRICTED);
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
				...
				streamFile(VoiceTrafficForwardAgiAudio.CREDIT_LIMIT_EXCEEDED);
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}		
				...
				streamFile(VoiceTrafficForwardAgiAudio.CALL_RATE_NOT_FOUND);
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
				...
				streamFile(VoiceTrafficForwardAgiAudio.CALL_NO_ENOUGH_BALANCE);
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
				...
				streamFile(VoiceTrafficForwardAgiAudio.REORDER);
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
				...
				this.streamFile(VoiceTrafficForwardAgiAudio.OVERDUE_INVOICE_LIMIT_EXCEEDED );
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
				...
				this.streamFile(VoiceTrafficForwardAgiAudio.CALL_UNATHORIZED);
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
				...
				this.streamFile(VoiceTrafficForwardAgiAudio.MAX_CONCURRENT_CALL_LIMIT_EXCEEDED);
				this.step = VoiceTrafficForwardAgiStep.STOP;
				return;
			}
	...
	public VoiceTrafficForwardAgiStep getStep() {
		return step;
	}
